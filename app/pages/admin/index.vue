...
.admin-dashboard__subtitle {
  @apply mt-1 text-slate-500 text-wrap;
}
...
.admin-dashboard__card {
  @apply flex items-center gap-4 rounded-3xl bg-white p-5 text-slate-700 shadow-sm transition hover:-translate-y-0.5 hover:shadow-md;
}

.admin-dashboard__card-icon {
  @apply flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-[--gold-one] text-white;
}

.admin-dashboard__card-label {
  @apply flex-1 text-base font-semibold text-slate-900;
}

.admin-dashboard__card-arrow {
  @apply h-4 w-4 text-slate-300 transition;
}
